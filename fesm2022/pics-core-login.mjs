...
let LoginComponent$1 = class LoginComponent {
    ...
    verifyOtp() {
        this.httpServie.post('/per-email/verifyotp', { "email": this.userEmail?.email, "otp": this.enterOTP }).subscribe((res) => {
            if (res && res.data.length) {
                this.profileService.setUserPreference();
                sessionStorage.setItem('user', JSON.stringify(this.userEmail));
                const landingPage = this.userEmail.userroles[0].defaultpage;
                ...
                this.alertService.error('Please enter valid verification code');
            }
        }, err => console.log(err));
    }
    sendOtp() {
            ...
            if (res.data) {
                ...
                this.authService.getUserRole(res.data.id).subscribe(userInfo => {
                    if (userInfo) {
                        ...
                        this.httpServie.post('/per-email/sendotp', { email: userInfo?.data?.email }).subscribe();
                    }
                    else {
                        this.isOTP = false;
                        this.profileService.setUserPreference();
